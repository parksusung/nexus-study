            ...
            resolve: async (src, args, ctx, info) => {
                try {
                    const a = await ctx.prisma.category.findMany({
                        where: {
                            OR: [
                                { c1_name: { contains: args.keyword } },
                                { c2_name: { contains: args.keyword } },
                                { c3_name: { contains: args.keyword } },
                                { c4_name: { contains: args.keyword } },
                            ]
                        },
                        orderBy: { id: "asc" }
                    });
        ...
        t.field("selectCategoriesByHierarchicalBySomeone", {
            ...
            resolve: async (src, args, ctx, info) => {
                try {
                    ...
                    const byConditionName = "c" + ((splitedCode?.length ?? 0) + 1) + "Name" as "c1_name" | "c2_name" | "c3_name" | "c4_name";
